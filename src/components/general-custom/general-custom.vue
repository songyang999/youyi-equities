...
-->
<template>
    <view :style="{ paddingTop: customHeaderHeight + 'px' }" class="page">
        <image src="/static/images/header_bac.png" class="general-custom-bac" />
        <view :style="{ height: customHeaderHeight + 'px' }" class="custom-head-bac">
            <image src="/static/images/header_bac.png" class="custom-bac" />
        </view>
        <!-- 自定义导航栏 -->
        <view :style="{ paddingTop: statusBarHeight + 'px' }" class="general-custom">
            <view :style="{ height: navigationBar + 'px' }" class="custom-head">
                <view v-if="hideBack" :style="{ width: navigationBar + 'px' }" class="custom-left" />
                <view v-else :style="{ width: navigationBar + 'px' }" class="custom-left" @click="navBack">
