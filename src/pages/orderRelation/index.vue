...
<template>
        ...
        <template #content>
            <view class="form-box">
                <uni-forms ref="ruleForm" :model-value="formData" :rules="rules" validate-trigger="blur" label-width="90">
                    ...
                    <button type="primary" hover-class="none" class="mb-30" @click="handleSubmit">确认关联</button>
                </uni-forms>
                <view class="explain_wp">
                    <view class="explain_title flex align-center justify-center mb-40">
                        <image src="/static/images/explain_left.png" mode="aspectFit" class="mr-6" />
                        <view class="fs-28 common_text">订单关联说明</view>
                        <image src="/static/images/explain_right.png" mode="aspectFit" class="ml-6" />
                    </view>
                    <view class="explain_li flex justify-between">
                        <view class="count mr-12">1</view>
                        <view class="sub_text fs-28 flex1">请输入订购产品时提供的手机号码</view>
                    </view>
                    <view class="explain_li flex justify-between">
                        <view class="count mr-12">2</view>
                        <view class="sub_text fs-28 flex1">需要接收回填验证码，以验明身份</view>
                    </view>
                    <view class="explain_li flex justify-between">
                        <view class="count mr-12">3</view>
                        <view class="sub_text fs-28 flex1">订单关联，即时生效，一个账户最多只能关联两个手机号下的订单</view>
                    </view>
                </view>
            </view>
        </template>
    ...
    <!-- 订购成功 -->
    <relation-success v-if="dialogVisible" :content="content" @close="closeSuccess" />
</template>
...
const handleRelevance = async () => {
    try {
        ...
        const res: any = await relevance(params);
